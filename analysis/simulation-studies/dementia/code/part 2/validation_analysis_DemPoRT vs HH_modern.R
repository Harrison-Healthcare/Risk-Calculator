...
# ----------------------------------------------------------------
# 2. Read final simulated dataset (HH modern)
# ----------------------------------------------------------------
sim_data <- readxl::read_excel("../simulation_dataset_final_HHmodern_DemPoRT.xlsx")
...
ggsave("scatter_DemPoRT_vs_HH_modern.png", p_scatter,
       width = 6, height = 4)
...
ggsave("residuals_vs_age_modern.png", p_resid_age, width = 6, height = 4)
...
ggsave("BA_overall_modern.png",      p_ba,      width = 7, height = 5)
ggsave("BA_by_Sex_modern.png",       p_ba_sex,  width = 7, height = 5)
ggsave("BA_by_Age_Group_modern.png", p_ba_age,  width = 7, height = 5)
...
ggsave("simple_slopes_bias_age_sex_modern.png", p_simple_slopes, width = 7, height = 5)
...
emm_df <- summary(emm_sex, infer = c(TRUE, TRUE)) %>%
  as.data.frame() %>%
  # Round means to 3dp, SE to 6dp, CI to 3dp, t/p left as-is
  mutate(
...
# Simple slopes of BA_Diff ~ Age_c by Sex (from the ANCOVA model)
slopes_emm  <- emtrends(ancova, ~ Sex, var = "Age_c")
slopes_sum  <- summary(slopes_emm, infer = c(TRUE, TRUE)) %>% as.data.frame()

# Clean/format: add percent-per-year for readability (reporting only)
slopes_tbl <- slopes_sum %>%
  rename(
    Age_Slope = Age_c.trend
  ) %>%
  mutate(
    Age_Slope_pp_per_year = Age_Slope * 100,    # percentage points / year
    Age_Slope   = round(Age_Slope, 6),
    SE          = round(SE, 6),
    df          = round(df, 0),
    t.ratio     = round(t.ratio, 3),
    p.value     = ifelse(p.value < 0.001, "<0.001", sprintf("%.3f", p.value)),
    lower.CL    = round(lower.CL, 6),
    upper.CL    = round(upper.CL, 6),
    # also keep pp/year for reference
    Age_Slope_pp_per_year = round(Age_Slope_pp_per_year, 3)
  )

# Optional: test Female vs Male slope difference
slope_contr <- summary(contrast(slopes_emm, method = "pairwise"), infer = c(TRUE, TRUE)) %>%
  as.data.frame() %>%
  mutate(
...
writexl::write_xlsx(
  list(
    ...
    Simple_Slopes_emtrends   = slopes_tbl,      # <-- NEW
    Slope_Contrasts          = slope_contr      # <-- NEW
  ),
  path = "validation_analysis_DemPoRT_HH_modern_summary_clean.xlsx"
)
